...
class marketDataHandler(DataHandlerLP):
    ...
    def __init__(
        ...
        **kwargs
        ...
        data_loader = {
            ...
            "kwargs": {
                "config": {
                    "feature": self.get_feature_config(market_index),
                },
    ...
    @staticmethod
    def get_feature_config(market_index):
        """
        ...
        """
        mi1 = market_index
        mi2 = 'sh000903' if market_index == 'sh000300' else market_index
        mi3 = 'sh000905' if market_index == 'sh000300' else market_index

        market_info = (
            [f'Mask($close/Ref($close,1)-1, "{mi1}")', f'Mask(Mean($close/Ref($close,1)-1,5), "{mi1}")', f'Mask(Std($close/Ref($close,1)-1,5), "{mi1}")', f'Mask(Mean($volume,5)/$volume, "{mi1}")', f'Mask(Std($volume,5)/$volume, "{mi1}")', f'Mask(Mean($close/Ref($close,1)-1,10), "{mi1}")', f'Mask(Std($close/Ref($close,1)-1,10), "{mi1}")', f'Mask(Mean($volume,10)/$volume, "{mi1}")', f'Mask(Std($volume,10)/$volume, "{mi1}")', f'Mask(Mean($close/Ref($close,1)-1,20), "{mi1}")', f'Mask(Std($close/Ref($close,1)-1,20), "{mi1}")', f'Mask(Mean($volume,20)/$volume, "{mi1}")', f'Mask(Std($volume,20)/$volume, "{mi1}")', f'Mask(Mean($close/Ref($close,1)-1,30), "{mi1}")', f'Mask(Std($close/Ref($close,1)-1,30), "{mi1}")', f'Mask(Mean($volume,30)/$volume, "{mi1}")', f'Mask(Std($volume,30)/$volume, "{mi1}")', f'Mask(Mean($close/Ref($close,1)-1,60), "{mi1}")', f'Mask(Std($close/Ref($close,1)-1,60), "{mi1}")', f'Mask(Mean($volume,60)/$volume, "{mi1}")', f'Mask(Std($volume,60)/$volume, "{mi1}")',
                f'Mask($close/Ref($close,1)-1, "{mi2}")', f'Mask(Mean($close/Ref($close,1)-1,5), "{mi2}")', f'Mask(Std($close/Ref($close,1)-1,5), "{mi2}")', f'Mask(Mean($volume,5)/$volume, "{mi2}")', f'Mask(Std($volume,5)/$volume, "{mi2}")', f'Mask(Mean($close/Ref($close,1)-1,10), "{mi2}")', f'Mask(Std($close/Ref($close,1)-1,10), "{mi2}")', f'Mask(Mean($volume,10)/$volume, "{mi2}")', f'Mask(Std($volume,10)/$volume, "{mi2}")', f'Mask(Mean($close/Ref($close,1)-1,20), "{mi2}")', f'Mask(Std($close/Ref($close,1)-1,20), "{mi2}")', f'Mask(Mean($volume,20)/$volume, "{mi2}")', f'Mask(Std($volume,20)/$volume, "{mi2}")', f'Mask(Mean($close/Ref($close,1)-1,30), "{mi2}")', f'Mask(Std($close/Ref($close,1)-1,30), "{mi2}")', f'Mask(Mean($volume,30)/$volume, "{mi2}")', f'Mask(Std($volume,30)/$volume, "{mi2}")', f'Mask(Mean($close/Ref($close,1)-1,60), "{mi2}")', f'Mask(Std($close/Ref($close,1)-1,60), "{mi2}")', f'Mask(Mean($volume,60)/$volume, "{mi2}")', f'Mask(Std($volume,60)/$volume, "{mi2}")',
                f'Mask($close/Ref($close,1)-1, "{mi3}")', f'Mask(Mean($close/Ref($close,1)-1,5), "{mi3}")', f'Mask(Std($close/Ref($close,1)-1,5), "{mi3}")', f'Mask(Mean($volume,5)/$volume, "{mi3}")', f'Mask(Std($volume,5)/$volume, "{mi3}")', f'Mask(Mean($close/Ref($close,1)-1,10), "{mi3}")', f'Mask(Std($close/Ref($close,1)-1,10), "{mi3}")', f'Mask(Mean($volume,10)/$volume, "{mi3}")', f'Mask(Std($volume,10)/$volume, "{mi3}")', f'Mask(Mean($close/Ref($close,1)-1,20), "{mi3}")', f'Mask(Std($close/Ref($close,1)-1,20), "{mi3}")', f'Mask(Mean($volume,20)/$volume, "{mi3}")', f'Mask(Std($volume,20)/$volume, "{mi3}")', f'Mask(Mean($close/Ref($close,1)-1,30), "{mi3}")', f'Mask(Std($close/Ref($close,1)-1,30), "{mi3}")', f'Mask(Mean($volume,30)/$volume, "{mi3}")', f'Mask(Std($volume,30)/$volume, "{mi3}")', f'Mask(Mean($close/Ref($close,1)-1,60), "{mi3}")', f'Mask(Std($close/Ref($close,1)-1,60), "{mi3}")', f'Mask(Mean($volume,60)/$volume, "{mi3}")', f'Mask(Std($volume,60)/$volume, "{mi3}")'],
            [f'Mask($close/Ref($close,1)-1, "{mi1}")', f'Mask(Mean($close/Ref($close,1)-1,5), "{mi1}")', f'Mask(Std($close/Ref($close,1)-1,5), "{mi1}")', f'Mask(Mean($volume,5)/$volume, "{mi1}")', f'Mask(Std($volume,5)/$volume, "{mi1}")', f'Mask(Mean($close/Ref($close,1)-1,10), "{mi1}")', f'Mask(Std($close/Ref($close,1)-1,10), "{mi1}")', f'Mask(Mean($volume,10)/$volume, "{mi1}")', f'Mask(Std($volume,10)/$volume, "{mi1}")', f'Mask(Mean($close/Ref($close,1)-1,20), "{mi1}")', f'Mask(Std($close/Ref($close,1)-1,20), "{mi1}")', f'Mask(Mean($volume,20)/$volume, "{mi1}")', f'Mask(Std($volume,20)/$volume, "{mi1}")', f'Mask(Mean($close/Ref($close,1)-1,30), "{mi1}")', f'Mask(Std($close/Ref($close,1)-1,30), "{mi1}")', f'Mask(Mean($volume,30)/$volume, "{mi1}")', f'Mask(Std($volume,30)/$volume, "{mi1}")', f'Mask(Mean($close/Ref($close,1)-1,60), "{mi1}")', f'Mask(Std($close/Ref($close,1)-1,60), "{mi1}")', f'Mask(Mean($volume,60)/$volume, "{mi1}")', f'Mask(Std($volume,60)/$volume, "{mi1}")',
                f'Mask($close/Ref($close,1)-1, "{mi2}")', f'Mask(Mean($close/Ref($close,1)-1,5), "{mi2}")', f'Mask(Std($close/Ref($close,1)-1,5), "{mi2}")', f'Mask(Mean($volume,5)/$volume, "{mi2}")', f'Mask(Std($volume,5)/$volume, "{mi2}")', f'Mask(Mean($close/Ref($close,1)-1,10), "{mi2}")', f'Mask(Std($close/Ref($close,1)-1,10), "{mi2}")', f'Mask(Mean($volume,10)/$volume, "{mi2}")', f'Mask(Std($volume,10)/$volume, "{mi2}")', f'Mask(Mean($close/Ref($close,1)-1,20), "{mi2}")', f'Mask(Std($close/Ref($close,1)-1,20), "{mi2}")', f'Mask(Mean($volume,20)/$volume, "{mi2}")', f'Mask(Std($volume,20)/$volume, "{mi2}")', f'Mask(Mean($close/Ref($close,1)-1,30), "{mi2}")', f'Mask(Std($close/Ref($close,1)-1,30), "{mi2}")', f'Mask(Mean($volume,30)/$volume, "{mi2}")', f'Mask(Std($volume,30)/$volume, "{mi2}")', f'Mask(Mean($close/Ref($close,1)-1,60), "{mi2}")', f'Mask(Std($close/Ref($close,1)-1,60), "{mi2}")', f'Mask(Mean($volume,60)/$volume, "{mi2}")', f'Mask(Std($volume,60)/$volume, "{mi2}")',
                f'Mask($close/Ref($close,1)-1, "{mi3}")', f'Mask(Mean($close/Ref($close,1)-1,5), "{mi3}")', f'Mask(Std($close/Ref($close,1)-1,5), "{mi3}")', f'Mask(Mean($volume,5)/$volume, "{mi3}")', f'Mask(Std($volume,5)/$volume, "{mi3}")', f'Mask(Mean($close/Ref($close,1)-1,10), "{mi3}")', f'Mask(Std($close/Ref($close,1)-1,10), "{mi3}")', f'Mask(Mean($volume,10)/$volume, "{mi3}")', f'Mask(Std($volume,10)/$volume, "{mi3}")', f'Mask(Mean($close/Ref($close,1)-1,20), "{mi3}")', f'Mask(Std($close/Ref($close,1)-1,20), "{mi3}")', f'Mask(Mean($volume,20)/$volume, "{mi3}")', f'Mask(Std($volume,20)/$volume, "{mi3}")', f'Mask(Mean($close/Ref($close,1)-1,30), "{mi3}")', f'Mask(Std($close/Ref($close,1)-1,30), "{mi3}")', f'Mask(Mean($volume,30)/$volume, "{mi3}")', f'Mask(Std($volume,30)/$volume, "{mi3}")', f'Mask(Mean($close/Ref($close,1)-1,60), "{mi3}")', f'Mask(Std($close/Ref($close,1)-1,60), "{mi3}")', f'Mask(Mean($volume,60)/$volume, "{mi3}")', f'Mask(Std($volume,60)/$volume, "{mi3}")']
        )
...
class MASTERTSDatasetH(TSDatasetH):
    ...
    def get_market_information(
        ...
        # print('ma dataset', self.market_dataset)
