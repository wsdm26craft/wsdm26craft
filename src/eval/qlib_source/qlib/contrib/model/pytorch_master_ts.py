...
class DailyBatchSamplerRandom(Sampler):
    def __init__(self, data_source, shuffle=False):
        ...
        self.daily_count = pd.Series(index=self.data_source.get_index(), dtype=np.float64).groupby("datetime").size().values
    ...
    def fit(self, dataset: DatasetH):
        ...
        for step in range(self.n_epochs):
            ...
            if train_loss <= self.train_stop_loss_thred:
                break
        torch.save(best_param, f'{self.save_path}master_{self.save_prefix}_{self.seed}.pkl')

    def predict(self, dataset: DatasetH, use_pretrained = True):
        if use_pretrained:
            self.load_param(f'{self.save_path}master_{self.save_prefix}_{self.seed}.pkl')
        if not self.fitted:
